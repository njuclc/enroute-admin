<template>
  <div>
    <div class="gva-search-box">
      ...
      <el-date-picker v-model="searchInfo.endCreatedAt" type="datetime" placeholder="结束日期" :disabled-date="time=> searchInfo.startCreatedAt ? time.getTime() < searchInfo.startCreatedAt.getTime() : false"></el-date-picker>
           ...
           <el-form-item label="数据源" prop="source">
            <el-select v-model="searchInfo.source" clearable placeholder="请选择" @clear="()=>{searchInfo.source=undefined}">
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="数据源" prop="source" width="120">
            <template #default="scope">
            {{ filterDict(scope.row.source,stringOptions) }}
            </template>
        </el-table-column>
          ...
          <el-table-column label="导出结果" width="200">
              <template #default="scope">
                  [JSON]
              </template>
          </el-table-column>
        ...
        <el-table-column align="left" label="操作" fixed="right" min-width="240">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="updateWarehouseExportTaskFunc(scope.row)">变更</el-button>
    ...
    <el-drawer size="800" v-model="detailShow" :before-close="closeDetailShow" title="查看详情" destroy-on-close>
        ...
        <el-descriptions :column="1" border>
                <el-descriptions-item label="数据源">
                        {{ filterDict(formData.source,stringOptions) }}
...
import {
  ...
  deleteWarehouseExportTask,
  deleteWarehouseExportTaskByIds,
...
const deleteRow = (row) => {
    ElMessageBox.confirm('确定要删除吗?', '提示', {
        ...
        type: 'warning'
            ...
            deleteWarehouseExportTaskFunc(row)
        })
    }
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteWarehouseExportTaskByIds({ IDs })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateWarehouseExportTaskFunc = async(row) => {
    const res = await findWarehouseExportTask({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteWarehouseExportTaskFunc = async (row) => {
    const res = await deleteWarehouseExportTask({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
